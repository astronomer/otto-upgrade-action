...
3. **KB coverage gate** (real runs) — validate every target against Otto's Upgrade KB through the same Core endpoint the migration uses. A target the KB can't migrate yet (a new Airflow version released hours ago, a provider version the KB hasn't ingested, a curated yank) is stepped down or held with the reason in the PR — the action never proposes what Otto can't reason about. The gate re-runs whenever pin reconciliation changes a target. Granularity is the Airflow version: a Runtime *build*-only move on the same Airflow (e.g. `3.3-1` → `3.3-2`) has no migration surface in the KB and isn't gated. Tokenless dry-runs preview the unchecked plan and say so.
